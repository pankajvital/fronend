...
const FlightResult = () => {
  ...
  // T//he empty dependency array ensures that the effect runs once when the component mounts
 ...
 const toggleItem = async (offer, index) => {
  ...
  //     const iatadeparture = segment.departure.iataCode;
  //     const iataarrival = segment.arrival.iataCode;

      
  //     if (!countryNames[iatadeparture]) {
  //       try {
  //         const originApiUrlDeparture = `https://test.api.amadeus.com/v1/reference-data/locations?subType=CITY,AIRPORT&keyword=${iatadeparture}`;
  //         const departureResponse = await axios.get(originApiUrlDeparture, { headers });
  //         const departureLocation = departureResponse.data?.data[0];

  //         if (departureLocation) {
  //           newCountryNames[iatadeparture] = departureLocation.detailedName;
  //         } else {
  //           newCountryNames[iatadeparture] = "Full Name Not Found";
  //         }
  //         await delay(500); // Adjust the delay time as needed (e.g., 1000ms = 1 second)

  //       } catch (apiError) {
  //         console.error("Origin API Error (Departure):", apiError);
  //         newCountryNames[iatadeparture] = "Error Fetching Full Name (Departure)";
  //       }
  //     }

  //     if (!countryNames[iataarrival]) {
  //       try {
  //         const originApiUrlArrival = `https://test.api.amadeus.com/v1/reference-data/locations?subType=CITY,AIRPORT&keyword=${iataarrival}`;
  //         const arrivalResponse = await axios.get(originApiUrlArrival, { headers });
  //         const arrivalLocation = arrivalResponse.data?.data[0];

  //         if (arrivalLocation) {
  //           newCountryNames[iataarrival] = arrivalLocation.detailedName;
  //         } else {
  //           newCountryNames[iataarrival] = "Full Name Not Found";
  //         }
  //         await delay(500); // Adjust the delay time as needed (e.g., 1000ms = 1 second)

  //       } catch (apiError) {
  //         console.error("Origin API Error (Arrival):", apiError);
  //         newCountryNames[iataarrival] = "Error Fetching Full Name (Arrival)";
  //       }
  //     }
  //   }
  // }

  // setCountryNames(prevCountryNames => ({
  //   ...prevCountryNames,
  //   ...newCountryNames,
  // }));

...
//       let apiUrlB = `https://test.api.amadeus.com/v2/shopping/flight-offers?originLocationCode=${location.state.allTripDetails[1].origin}&destinationLocationCode=${location.state.allTripDetails[1].destination}&departureDate=${location.state.allTripDetails[0].to}&adults=${location.state.tripDetailsPassenger.passengerData.adults}&children=${location.state.tripDetailsPassenger.passengerData.children}&infants=${location.state.tripDetailsPassenger.passengerData.infant}&travelClass=${location.state.allTripDetails[0].cabin}&currencyCode=USD&max`;


//       console.log("API URL:", apiUrl);

//       // //Check if 'to' exists in location.state.allTripDetails
//       if (location.state.allTripDetails.to) {
//         apiUrl += `&returnDate=${location.state.allTripDetails.to}`;
...
//         });
//     }
//   }, [accessToken, isLoading, error, location.state.allTripDetails]); // Include 'location.state.allTripDetails' in the dependency array
...
useEffect(() => {
    if (isLoading) {
      ...
      const [tripDetailsA, tripDetailsB] = location.state.allTripDetails;
  
      let apiUrlA = `https://test.api.amadeus.com/v2/shopping/flight-offers?originLocationCode=${location.state.allTripDetails[0].origin}&destinationLocationCode=${location.state.allTripDetails[0].destination}&departureDate=${location.state.allTripDetails[0].from}&adults=${location.state.tripDetailsPassenger.passengerData.adults}&children=${location.state.tripDetailsPassenger.passengerData.children}&infants=${location.state.tripDetailsPassenger.passengerData.infant}&travelClass=${location.state.allTripDetails[0].cabin}&currencyCode=USD&max`;
    ...
    }, [accessToken, isLoading, error, location.state]);
  ...
  useEffect(() => {
    if (selectedDataIndex !== null && displayedData && displayedData.length > selectedDataIndex) {
      ...
      <div className="search-img top-page">
        <div className="loader">
          ...
          <div className="iata_code departure_city">
            {location.state.allTripDetails.origin}
          </div>
          <div className="plane">
            <img
          ...
          <div className="iata_code arrival_city iata_code arrival_city-d">
            {location.state.allTripDetails.destination}
          </div>
        </div>
      </div>
    );
  ...
  return (
    <>
      ...
      {/* Flight Search Areas */}
      <section id="explore_area" className="mt-5">
                  ...
                  <div className="filter-price">
                    <div>
                      ...
                      {filteredData.map((item) => (
                        <div key={item.id}>
                          {/* Render flight allTripDetails here */}
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      ? apiData.data.filter((item) =>
          item.itineraries.some(
            (itinerary) =>
              ...
              itinerary.segments[0].carrierCode === carrier
          )
        ).length
      : apiData.data.filter((item) =>
          item.itineraries.some(
            (itinerary) =>
              itinerary.segments &&
              (itinerary.segments.length > 0 ||
                itinerary.segments.length > 1) && // This line considers both 0 and 1 segments
              (itinerary.segments[0].carrierCode === carrier ||
                itinerary.segments[1]?.carrierCode === carrier) // Handling 0 and 1 segments
          )
        ).length
  }
</span>
            ...
            <div className="col-lg-9">
                ...
                <div className="mb-3">
                    {isLoading ? (
                      ...
                      <>
                        ...
                        <p className="total-tet">New York: All Airports USA 
                        {/* {location.state.allTripDetails[0].origin} */}
                         to Miami: International USA (MIA) Thu 21,Dec 2023 - Sun 31,Dec 2023</p>
                        <p className="total-res">{totalResultsCount} Results found</p>
                        </div>
                      </>
                    ) : null}
                  </div>
              ...
              {apiData &&
                ...
                (allUnchecked || selectedCarriers.length === 0
                  ...
                  (displayedData ? displayedData.slice(0, displayedResults) : apiData.data.slice(0, displayedResults)) //  displayed results
                    ...
                    return (
                      ...
                      >
                        ...
                        {offer.itineraries.map((itinerary, index) => {
                          ...
                          // console.log('jai sssssssssssssssssssssssssssssssssss', offer)
                          ...
                          const content = index === 0 ? 'Out bound' : index === 1 ? 'In bound' : 'Some default content';
                            ...
                            <div className="mt-3">
                              {/* <div className="flight_search_result_wrapper">
                                <div className="flight_search_item_wrappper">
                                  <div className="flight_search_itemsss">
                              ...
                              <div className="row">
                                ...
                                <div className="flight_search_middel offset-md-1 col-md-5 justify-content-between">
                                  ...
                                  <span className="arrow-text">{content}</span>
                                    <br/>
                                    <img
                        ...
                        <div className="col-md-12 mb-3">
                          <div className="show-allTripDetails">
                            <h6
                              className="btn mybtn"
                                  ...
                                  <div className="modal-content">
                                    ...
                                    <div className="modal-body">
                                      <table class="table">
                                        ...
                                        <tbody>
                                          {apiData.data && apiData.data.some(item => item.itineraries && item.itineraries.length > 0) && (
                                            <tr>
                                              <td>{location.state.allTripDetails[0].origin} - {location.state.allTripDetails[0].destination}</td>
                                              <td>Pieces: 0</td>
                                              <td>7 Kg (Adult)</td>
                                            </tr>
                                          )}

                                          {apiData.data && apiData.data.some(item => item.itineraries && item.itineraries.length > 1) && (
                                            <tr>
                                              <td>{location.state.allTripDetails[0].destination} - {location.state.allTripDetails[1].destination}</td>
                                              <td>Pieces: 0</td>
                                              <td>7 Kg (Adult)</td>
                                            </tr>
                                          )}
            ...
            : 'Not Available';
                                        ...
                                        return (
                                          <>
                                            ...
                                            >
                                              <div className="airline-allTripDetails">
                                                  ...
                                                  <div className="flight_det">
                                                    <div className="code_time">
                                                      <span className="code">
                                                        ...
                                                        {/* <GetFullName origin={segment.departure.iataCode}/> */}
                                                      </span>
                                                      {/* {airlineNameD} */}
                                                      <span className="time">
                                                        {
